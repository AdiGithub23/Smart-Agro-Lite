...
exports.getGraphData = async (req, res) => {
  try {
    ...
    // Find the latest sensor data to get the recorded_at time
    ...
    // If no sensor data is found, return 48 points with null time and value
    let latestTime;
    if (!latestSensorData) {
      latestTime = new Date(); // Use current time as fallback
    } else {
      latestTime = new Date(latestSensorData.recorded_at);
    }

    const intervalDuration = 30 * 60 * 1000; // 30-minute intervals
    ...
    // Iterate over all parameters from the device package
    for (const parameterName of parameters) {
      const thresholdParam = thresholdMapping[parameterName];
      const sensorColumn = sensorColumnMapping[parameterName];

      // If the parameter is mapped to thresholds
      if (thresholdParam) {
        ...
        // Generate 48 data points for each parameter
        for (let i = 0; i < 48; i++) {
          const intervalEnd = new Date(
            currentIntervalStart.getTime() - intervalDuration
          );

          // If latestSensorData doesn't exist, generate null data points
          if (!latestSensorData) {
            ...
            const sensorData = await Sensor.findAll({
              where: {
                deviceId: id,
                recorded_at: {
                  [Op.between]: [intervalEnd, currentIntervalStart],
                },
              },
            ...
            if (sensorData.length > 0) {
              const averageValue =
                sensorData.reduce(
                  (sum, entry) => sum + entry[sensorColumn],
                  0
                ) / sensorData.length;
              const time = currentIntervalStart
                .toISOString()
                .slice(0, 16)
                .replace("T", " ");
